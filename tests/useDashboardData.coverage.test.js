...
describe('useDashboardData coverage', () => {
  ...
  it('calcula saldo tras meta como acumulado de disponible gastos hasta el mes actual', async () => {
    const mesActual = resultMesActual();
    const mesesPrevios = ['Enero', 'Febrero', 'Marzo', 'Abril'].filter((mes) => mes !== mesActual);
    setGetJsonImplementation({
      ...
      '/resumen_estimado_vs_real': () =>
        Promise.resolve([
          ...mesesPrevios.map((mes, index) => ({
            mes,
            disponible_gastos: [-1110100, 205527, -3625339.2, -1835119][index],
          })),
          { mes: mesActual, disponible_gastos: -159336 },
        ]),
    });
